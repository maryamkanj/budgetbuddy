...
export function GoalsList({ initialGoals = [] }: { initialGoals?: Database['public']['Tables']['goals']['Row'][] }) {
  ...
  const [filter, setFilter] = useState<'all' | 'active' | 'completed' | 'failed' | 'overdue'>('all');

  const filteredGoals = useMemo(() => {
    return goals.filter(goal => {
      if (filter === 'all') return true;
      if (filter === 'overdue') return getGoalEffectiveStatus(goal) === 'Overdue';
  ...
  const stats = useMemo(() => {
    return {
      total: goals.length,
      active: goals.filter(g => getGoalEffectiveStatus(g) === 'Active').length,
      completed: goals.filter(g => getGoalEffectiveStatus(g) === 'Completed').length,
      overdue: goals.filter(g => getGoalEffectiveStatus(g) === 'Overdue').length,
      failed: goals.filter(g => getGoalEffectiveStatus(g) === 'Failed').length,
    };
  }, [goals]);
    ...
    <div className="space-y-8">
      ...
      <Card className="rounded-xl border border-border bg-card/40 backdrop-blur-md p-2">
        <div className="flex flex-wrap gap-2">
          {(['all', 'active', 'completed', 'failed', 'overdue'] as const).map((opt) => (
            <Button
              key={opt}
        ...
        {filteredGoals.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredGoals.map((goal) => {
              ...
              const effectiveStatus = getGoalEffectiveStatus(goal);
                  ...
                  <CardHeader className="pb-4 pt-6 px-6">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className={daysRemaining !== null && daysRemaining < 0 ? 'text-rose-400 font-medium' : 'text-muted-foreground'}>
                          {deadlineStatus}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-xs font-bold uppercase tracking-widest">
                          ...
                          <span className={effectiveStatus === 'Overdue' ? 'text-rose-400' : 'text-primary'}>
                            {Math.round(progress)}%
                          </span>
                        </div>
                        <Progress 
                          ...
                          className={`h-3 rounded-full ${
                            effectiveStatus === 'Overdue' ? 'bg-rose-500/20' : 'bg-white/5'
                          }`} 
                        />
                      </div>
                    </div>
