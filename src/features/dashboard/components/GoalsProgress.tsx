...
export function GoalsProgress({ goals, limit = 3 }: GoalsProgressProps) {
  ...
  const activeGoals = goals.filter(goal => goal.status === 'Active' && goal.current_amount < goal.target_amount);
            ...
            <div key={goal.id} className="space-y-3 p-3 sm:p-4 border border-white/5 rounded-xl sm:rounded-2xl bg-card/40 hover:bg-card/60 transition-all duration-300 group relative overflow-hidden">
              ...
              <div className="flex justify-between items-start relative z-10">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {daysLeft !== null && (
                    <span className={`text-[10px] px-2 py-0.5 rounded-full ${daysLeft < 7 ? 'bg-card text-muted-foreground' :
                      daysLeft < 30 ? 'bg-accent/10 text-accent' :
              ...
              <div className="text-right text-[10px] font-bold text-primary uppercase tracking-tighter">
